...
from ...models import Website,Add_website
import requests
import os
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not (Website.objects.filter(user=_website.split("/")[0], repository=_website.split("/")[1]).exists()):
            try:
                if(Add_website.objects.filter(user=_website.split("/")[0], repository=_website.split("/")[1]).exists()):
                    _web = Add_website.objects.get(user=_website.split(
                    ...
                    self.request(web)
                    Add_website.objects.get(user=_website.split(
